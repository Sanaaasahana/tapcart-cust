...
export function PaymentPage({ onSuccess, onBack }: PaymentPageProps) {
  ...
  const submitPurchaseToStore = async (transactionId: string, paymentMethod: string) => {
    try {
      const phone = typeof window !== "undefined" ? sessionStorage.getItem("userPhone") || "" : ""
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      const { createPaymentOrder, verifyPayment } = await import("@/lib/store-api")
      const orderData = await createPaymentOrder(storeId, finalTotal, "INR")
        ...
        const options = {
          ...
          handler: async (response: any) => {
            try {
              ...
              const transactionId = verifyData.transactionId || response.razorpay_payment_id

              // Submit purchase to store
              await submitPurchaseToStore(transactionId, "Credit/Debit Card")
  ...
  const handleUPIPayment = async () => {
    ...
    try {
      ...
      const storeId = items[0].storeId
      const response = await initiateUPIPayment(upiId, finalTotal, "Checkout Payment", storeId)
      ...
      if (response.success) {
        ...
        // Submit purchase to store
        try {
          await submitPurchaseToStore(transactionId, "UPI")
        } catch (err) {
          console.error("Failed to submit purchase:", err)
          // Still proceed to success page even if store submission fails
        }
  ...
  const handleCounterPayment = async () => {
    ...
    try {
      ...
      // Submit purchase to store
      await submitPurchaseToStore(transactionId, "Pay at Counter")
